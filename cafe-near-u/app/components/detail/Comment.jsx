...
import useDeleteComment from "../../ApiHook/useFetchInfoComment";
...
export default function Comment({ comment }) {
  const { id, rating, created_at, context, user } = comment;
  ...
  const handleDeleteComment = async (id) => {
    ...
    try {
      const response = await fetch(
        `https://13.211.10.154/api/1.0/shops/${cafeid}/comments/${id}`,
        {
          method: "DELETE",
          headers: {
  ...
  return (
    <Card className="rounded-md h-auto group relative">
      {user.id == userid && (
        <div
          className="absolute top-4 right-4 text-gray-400 group-hover:text-gray-600 cursor-pointer hidden group-hover:inline"
      ...
      <div className="flex flex-row items-start p-4 gap-2 w-[90%]">
      ...
      <div className="px-6 py-2 pb-6">{context}</div>
    </Card>
  );
}
