...
export default function StoreCard({ className, store }) {
    ...
    return (
        <div
            className={`relative flex flex-col justify-start cursor-pointer  ${className}`}
        >
            <Carousel transition={{ duration: 2 }} className="h-auto mb-2">
                <img
                    src={primary_image}
            ...
            <div
                className="text-2xl my-2 text-gray-800"
                onClick={() => handletostoreinfo(store.id)}
            >
                {name}
            </div>
